...
class Laundry {
	...
	private float detergentBill;
	private float waterBill;
	private Cloth[] clothes;

	public Laundry(float electricityBill, float detergentBill, float waterBill, Cloth[] clothes) {
		super();
		this.electricityBill = electricityBill;
		this.detergentBill = detergentBill;
	...
	public float getDetergentBill() {
		return detergentBill;
	}
	...
	public void setDetergentBill(float detergentBill) {
		this.detergentBill = detergentBill;
	}
	...
	public float calculateTotalBill() {
		return detergentBill + waterBill + electricityBill;
	}

	public void printLaundryDetails() {
		System.out.println("Electricity Bill: $" + electricityBill);
		System.out.println("Detergent Bill: $" + detergentBill);
...
class WashingMachine extends Machine // isA
{
	...
	// usesA usesA usesA
	Laundry wash(Detergent detergent, Water water, Electricity electricity, Cloth clothes[]) {
		for (int i = 0; i < clothes.length; i++) {
			System.out.println("> Washing machine is washing..." + clothes[i].getClothColor() + " color "
					+ clothes[i].getClothMaterial() + " " + clothes[i].getClothType() + " of cost " + clothes[i].getCost() + " Rs.");
		}
		System.out.println("> It is using detergent of " + detergent.getBrand() + " QTY " + detergent.getQuantity());
		System.out.println("> Water type is " + water.getType() + " of temp " + water.getTemperatureValue());
		System.out.println("> Using electricity from : " + electricity.getVendor() + " of voltage : " + electricity.getVoltage());
		theTub.spin();
		float detergentBill = detergent.getQuantity() * detergent.getAmount();
		float waterBill = water.getQuantity() * 0.5f;
		float electricityBill = electricity.getCostPerUnit() * electricity.getUnitConsumed();
		return new Laundry(electricityBill, detergentBill, waterBill, clothes);
	}
}